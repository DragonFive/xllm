...
### 1.2 从 `xGR` 论文看，这个场景最难的 3 个问题

结合 `xGR.pdf` 的分析，生成式推荐服务和通用 LLM 推理相比，最值得单独拎出来的有 3 类问题。
...
### 1.3 这篇设计文档和 `xGR` 论文的关系

这篇文档不会复述整篇论文，也不试图替代论文中的实验部分。  
更准确的定位是：它把论文里抽象出来的工作负载特征和系统问题，映射到 xLLM 当前分支的真实实现上。

也就是说：

- 论文提供了问题定义和系统设计动机；
- 当前分支代码提供了这些设计在工程侧的具体落点；
- 本文档负责把这两者接起来，帮助后续做分享、走读和代码 review。
...
### 4.3 `xBeam`：beam search 的系统化处理

如果说 `xAttention` 解决的是“attention 如何更省显存、更少重复读取 Shared KV”，那么 `xBeam` 解决的就是“在大候选池下，beam search 怎么避免把时间浪费在无效比较和无谓排序上”。
...
因此，`xBeam` 这一层更适合被理解成“围绕 beam search 的系统优化”，而不是单纯一个排序 kernel。  
它的目标包括：
...
### 4.4 `xSchedule`：多级流水线和多流并发

从 `xGR` 论文的视角看，第三类问题不是某个单点算子慢，而是整个 pipeline 的分层协作方式不够适合生成式推荐。  
在当前设计里，这种思路可以抽象成 `xSchedule`：它不是一个单独的类名，而是一类系统层优化思想，用来描述“如何让 host、engine、worker、算子执行之间尽量重叠”。
...
因此，`xSchedule` 在本文档里可以理解为：把 fixed-step 场景下的调度稳定性、设备侧连续执行，以及多流并发能力组合起来的系统层设计。

### 4.5 从论文结果看，这套设计为什么值得做

`xGR.pdf` 给了一个很有价值的结论：在高并发、严格低时延约束下，围绕 `xAttention`、`xBeam`、`xSchedule` 做系统重构，可以带来明显吞吐收益。  
论文摘要里给出的结果是：在严格时延限制下，相比 baseline 至少取得 `3.49×` 的吞吐提升。

这并不意味着当前 xLLM 分支已经逐项复现了论文里的所有实验结论，但它至少说明了一件事：  
把生成式推荐单独当成一种特殊 workload 来设计调度、执行和算子，不是“过度工程化”，而是有明确收益空间的。
