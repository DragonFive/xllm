...
更准确的定位是：它把生成式推荐场景里的工作负载特征、系统问题和当前分支的真实实现路径整理到同一份设计文档里，帮助后续做走读、核对和代码 review。
...
如果从主执行链视角来展开，这条主链非常适合作为“架构总图”之后的第一条展开线，因为它把“固定调度、整图执行、定制算子”三件事串成了一个具体执行过程，而不是三个割裂的优化点。
...
这一节的目的，不是重复“代码结构”里的文件清单，而是给读者一个更可执行的阅读顺序：如果后续要继续走读代码、扩写文档，或者排查 `backend=rec` 路径上的行为差异，可以直接按下面的顺序进入。
...
这一层适合回答一个问题：为什么说 `backend=rec` 不是另起炉灶，而是接进了现有服务框架。

### 8.3 从调度和引擎看主链

如果这一节的重点是“为什么 fixed step 更适合 rec”，阅读顺序建议是：
...
如果要强调“fixed step 不是口头概念，而是代码主链的真实选择”，这一层就是核心证据。
...
如果想从“执行效率”而不是“调度策略”切入，这一层是最值得重点展开的。
...
也就是说，如果要把 `fixed_steps_scheduler`、`multi_step_pipeline`、`xAttention` 和 `beam search` 串成一条线，最终一定会落到这里。
...
这样逻辑最顺，也最适合作为文档内的主阅读路径。

## 9. 关键代码锚点索引

如果后续需要继续补代码注释，或者在评审时快速证明文档里的说法来自当前分支实现，可以直接从下面这组锚点入手。
...
如果需要明确“beam search 和 cache select 不是调度层概念，而是直接落到 device 热路径上的实现”，这一组锚点最适合引用。
...
前面的“代码路径附录”和“关键代码锚点索引”更像是资料库，适合在需要时回查；但如果这篇文档要真正成为一份开发者技术文档，还需要一条更偏“展开顺序”的线索。下面给出一个更适合正文组织的展开顺序。

### 10.1 推荐的展开顺序

如果读者并不直接参与 `backend=rec` 的实现，建议不要一上来就讲 `RecWorkerImpl` 或 `beam_search.cpp`，而是按下面的顺序推进：
...
5. **最后回到代码**
   - 再用代码主链和锚点证明前面的结论，不让正文停留在概念层

这个顺序的好处是：读者先理解“为什么”，再理解“怎么做”，最后再看“代码在哪里”。这比一开始就从实现文件名切入更容易跟上。

### 10.2 如果是内部走读，顺序可以反过来

如果阅读目标本身就是 xLLM 或推荐基础设施相关实现，那么也可以采用另一条顺序：
...
这种顺序更适合代码走读，因为它直接沿着调用栈往下走。但它的缺点是，对没有上下文的读者来说，一开始就会掉进实现细节里，不容易先抓住设计选择背后的动机。

### 10.3 建议优先明确的 3 个结论

如果后续需要把这篇文档压缩成更短的摘要，可以先把整篇内容收束成下面 3 句话：
...
这三句话可以作为整篇文档的核心摘要，后面的代码链路和锚点都可以理解为是在为这三句结论提供证据。
...
如果要进一步把这篇文档作为设计说明和实现说明的统一底稿，建议把验证项和设计目标一一对应起来。
...
- 使用层面：读者能够从文档中直接抽出主线，而不需要再次重构逻辑。

只有同时满足这几类条件，这篇文档才不只是“说明写过了”，而是真正能作为设计、实现说明、review 三者共用的稳定底稿。
...
这一节的作用，是避免 reviewer 或读者把“文档里讲得通”误以为“当前分支里一定已经逐项完全实现”。  
实现对齐表能让后续讨论更聚焦：哪些内容是已经可以拿代码证明的，哪些内容是当前设计的系统层解释。

## 16. 失败模式与可观测性

如果这篇文档只讲“正常路径”，那它仍然更像概念说明，而不是技术文档。在线系统一定要考虑失败模式与观测手段，因此这里补一个最小版本。
