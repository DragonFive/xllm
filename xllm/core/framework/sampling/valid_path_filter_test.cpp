...
TEST(ValidPathFilterTest, Vector) {
  ...
  ValidPathFilter filter = ValidPathFilter(tokens_list, vocab_size, dtype, device);
  ...
  // 验证输出形状
  EXPECT_EQ(mask.sizes(), torch::IntArrayRef({candidate_tokens.size(), vocab_size}));
...
TEST(ValidPathFilterTest, File) {
  ...
  for (const auto& row : tokens_list) {
    ...
    outfile.write(reinterpret_cast<const char*>(row.data()), row.size() * sizeof(int32_t));
    item_id++;
  }
  outfile.close();
  ...
  ValidPathFilter filter = ValidPathFilter(rec_tokens_file, vocab_size, dtype, device);
