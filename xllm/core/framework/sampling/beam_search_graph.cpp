...
bool BeamSearchGraph::capture(
  ...
  // 获取 persistent buffers（使用 bucket_batch 而非 actual batch）
  auto acc_logprob =
  ...
  // 创建 dummy out_beam_count_prefix_sums（beam_search 函数需要但不使用）
...
bool BeamSearchGraphExecutor::should_use_graph(uint32_t batch_size,
  ...
  if (!FLAGS_enable_beam_search_graph) {
    return false;
  }
...
BeamSearchGraphOutput BeamSearchGraphExecutor::forward(
  ...
  if (it == graphs_.end()) {
    ...
    // 捕获 graph
    bool success = graph->capture(batch_size,
                                  beam_size,
                                  current_step,
                                  total_rounds,
                                  bucket_batch,
